...
const common = require('../../api/v2/common');
...
const ClaimIntimation = require('../../models/claimIntimation');
...
router.get('/packages/get', xAccessToken.token, async function (req, res, next) {
    ...
    await UserPackage.find({}).cursor().eachAsync(async (package) => {
                ...
                if (car.fuel_type == "Petrol" || car.fuel_type == "Diesel") {
                    await Service.find({ model: car.model, fuel_type: car.fuel_type, })
                        .cursor().eachAsync(async (service) => {
                            var labour_cost = await q.all(packageDiscount(req.body.package, req.body.type, service.service, service.labour_cost, req.headers['tz']));
                            packages.push({
                                ...
                                criginalCost: service.part_cost + service.labour_cost,
                ...
                for (var i = 0; i < services.length; i++) {
                    if (services[i].type == "services") {
                        await Service.find({ _id: services[i].id }).cursor().eachAsync(async (service) => {
                            if (service) {
                                var labour_cost = await q.all(packageDiscount(req.body.package, services[i].type, service.service, service.labour_cost));
                        ...
                        if (paytmRes.STATUS == "TXN_SUCCESS") {
                            ...
                            await Booking.findOneAndUpdate({ _id: booking._id }, { $set: data }, { new: false }, async function (err, doc) {
                                if (err) {
                                    ...
                                    if (booking.package) {
                                        var package = await UserPackage.findOne({ _id: booking.package }).exec();
                                        if (package) {
                                            booking.services.forEach(async function (service) {
...
async function packageDiscount(p, c, s, lc, tz) {
    var labour_cost = lc;
    var package = await UserPackage.findOne({ _id: p }).exec();
    var packageUsed = await PackageUsed.find({ user: package.user, label: s }).count().exec();
    if (package) {
