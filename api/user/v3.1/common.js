    ...
    if (validation.fails()) {
        ...
        if (!checkUser) {
            ...
            User.findOneAndUpdate({ _id: req.body.id }, { $set: { account_info: data, otp: null } }, async function (err, doc) {
                ...
                else if (user.account_info.type == "driver") {
                    var business = await Management.find({ user: user._id }).exec();
                    var json = ({
                        responseCode: 200,
                        responseMessage: "OTP verified",
            ...
            if (user.account_info.type == req.body.type) {
                ...
                User.findOneAndUpdate({ _id: user._id }, { $set: updateData }, { new: true }, async function (err, doc) {
                    if (err) {
                        ...
                        User.findOneAndUpdate({ _id: user._id }, { $set: { otp: null } }, function (err, doc) { });
    ...
    if (validation.fails()) {
        ...
        if (car) {
            if (variant) {
                ...
                Car.findOneAndUpdate({ _id: req.body.car_id, user: user }, { $set: req.body }, { new: false }, async function (err, s) {
                    if (err) {
                        var json = ({
                            responseCode: 400,
                            responseMessage: "Error occured",
    ...
    if (userType) {
        ...
        await Booking.find({ 'user': user, 'status': { $ne: "Inactive" }, is_services: true })
            ...
            .cursor().eachAsync(async (booking) => {
                if (booking.status == "Pending" || booking.status == "Failure" || booking.status == "Confirmed" || booking.status == "Completed" || booking.status == "Approval" || booking.status == "Approved" || booking.status == "Cancelled") {
