...
router.get('/home', async function (req, res, next) {
    ...
    await Model.find({})
        .cursor().eachAsync(async (model) => {
            ...
            if (length >= 4267) {
                var service = await Service.find({ model: model._id, service: "Periodic Maintenance" }).count().exec();
                if (service == 0) {
                    packages.push({
                        model: model._id,
                        for: model.model,
...
router.get('/encryption/get', async function (req, res, next) {
    var key = config.key


    var data = 'DD0D07B77C1606C97A1B10AC8D9AC180'
    var data = 'welcome'
...
router.get('/refer-earn', xAccessToken.token, async function (req, res, next) {
    ...
    res.status(200).json({
        ...
        responseData: {
            ...
            user: 250,
            image: ["https://s3.ap-south-1.amazonaws.com/careager-staging/icon/car-refer-offer.jpg", "https://s3.ap-south-1.amazonaws.com/careager-staging/icon/careager-app.jpg"],
        }
    });
});
...
router.post('/booking/services/get', xAccessToken.token, async function (req, res, next) {
    ...
    if (validation.fails()) {
        ...
        if (car) {
            if (req.body.type == "services" && car.model.segment != "Small") {
                await Service.find({ model: car.model._id, })
            ...
            if (req.body.type == "collision" && car.model.segment != "Small") {
                await Collision.find({ model: car.model._id })
                    .cursor().eachAsync(async (service) => {
                ...
                await Detailing.find({ segment: car.model.segment })
                    .cursor().eachAsync(async (service) => {
                ...
                if (car.model.segment != "small") {
                    ...
                    await Package.find({ category: "addOn" }).cursor().eachAsync(async (service) => {
                        var serverTime = moment.tz(new Date(), req.headers['tz']);
                ...
                res.status(422).json({
                    responseCode: 422,
                    responseMessage: "Services are not available",
                    responseData: {},
                });
            }
        }
...
router.get('/booking/time-slot/', xAccessToken.token, async function (req, res, next) {
    if (req.query.business && req.query.date) {
        var slots = [];
        ...
        await BookingTiming.find({ business: business })
            .sort({ sort: 1 })
            .cursor().eachAsync(async (timing) => {
                var slot = await Booking.find({
                    time_slot: timing.slot,
...
router.post('/booking/package/add/', xAccessToken.token, async function (req, res, next) {
    ...
    if (validation.fails()) {
        ...
        if (checkCar && checkPackage) {
            ...
            await Detailing.find({ service: req.body.label, model: checkCar.model }).cursor().eachAsync(async (service) => {
                if (service) {
                    var labour_cost = await q.all(packageDiscount(req.body.package, checkCar._id, "collision", service.service, Math.ceil(service.labour_cost)));
            ...
            await Customization.find({ service: req.body.label, model: checkCar.model }).cursor().eachAsync(async (service) => {
                if (service) {
                    var labour_cost = await q.all(packageDiscount(req.body.package, checkCar._id, "collision", service.service, Math.ceil(service.labour_cost)));
                                ...
                                if (limit != coupon.limit) {
                                    if (coupon.for == "general") {
                                        ...
                                        if (Object.keys(coupon_for).length > 0) {
                                            ...
                                            labour_cost = labour_cost - coupon_labour_cost;
                                        ...
                                        if (Object.keys(coupon_for).length > 0) {
                                            ...
                                            var coupon_labour_cost = _.sumBy(coupon_for, x => x.labour_cost);
                                            var labour_cost = _.sumBy(booking.services, x => x.labour_cost);
                                            var coupon_part_cost = _.sumBy(coupon_for, x => x.part_cost);
                                            var part_cost = _.sumBy(booking.services, x => x.part_cost);
                                            ...
                                            if (coupon.type == "fixed") {
                                                discount = (coupon_labour_cost + coupon_part_cost) - coupon.discount;
                                                var paid_total = part_cost + labour_cost - (coupon_labour_cost + coupon_part_cost) + coupon.discount;

                                                // console.log(part_cost + " " + labour_cost + " " + coupon_labour_cost + " " + coupon_part_cost)
                                                var pick_up_charge = 0
                                                if (booking.convenience) {
                                                    if (booking.convenience != "Self Drop") {
                                                        if (paid_total <= pick_up_limit) {
                                                ...
                                                data = {
                                                    payment: {
                                                        ...
                                                        pick_up_limit: Math.ceil(booking.payment.pick_up_limit),
                                                        pick_up_charges: Math.ceil(pick_up_charge),
                                                        discount_total: Math.ceil(discount),
                                                        labour_cost: Math.ceil(booking.payment.labour_cost),
                                                        part_cost: Math.ceil(booking.payment.part_cost),
                                                        paid_total: Math.ceil(paid_total),
                                                        total: Math.ceil(booking.payment.total),
...
router.get('/coins/transaction/get', xAccessToken.token, async function (req, res, next) {
    ...
    await Point.find({ user: user }).sort({ created_at: -1 }).cursor().eachAsync(async (point) => {
        ...
        if (point.tag == "commission") {
            ...
            var tag = "Commission - " + booking.user.name + " - #" + booking.booking_no;
        }
