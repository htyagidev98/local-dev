...
router.get('/home', async function (req, res, next) {
    ...
    {   
        var model = await Model.find({}).exec();
    }*/

    res.json(await Model.find({}).exec())
    ...
    /* await Collision.find({model:"5bb78fda9400270f421a54eb"})
     .cursor().eachAsync(async(service) => {        
         var data = {
             "automaker": "5bb7717891d4227629cd25e4",
             "maker": "Honda",
             "model": "5c9f194ff0e5af1d17857412",
             "for": "Civic 2019",
        ...
        if (checkUser) {
            if (checkUser.account_info.status == "Complete") {
                ...
                User.findOneAndUpdate({ _id: checkUser._id }, { $set: data }, { new: true }, async function (err, doc) {
                    var user = await User.findOne({ _id: checkUsers._id }).exec();
                    event.otpSms(user);
    ...
    if (validation.fails()) {
        ...
        if (car) {
            if (req.body.type == "services") {
                if (car.model.segment != "Small") {
                    await Service.find({ segment: car.model.segment, publish: true, })
                        .cursor().eachAsync(async (service) => {
            ...
            if (req.body.type == "collision") {
                if (car.model.segment != "Small") {
                    await Collision.find({ segment: car.model.segment, publish: true })
                        .cursor().eachAsync(async (service) => {
                ...
                if (car.model.segment != "Small") {
                    await Detailing.find({ segment: car.model.segment, publish: true, })
                        .cursor().eachAsync(async (service) => {
                            var gallery = service.gallery;
                            var getDiscount = {
                                package: req.body.package,
                ...
                if (car.model.segment != "Small") {
                    await Customization.find({ model: car.model._id, publish: true, })
                        .cursor().eachAsync(async (service) => {
    ...
    if (validation.fails()) {
        ...
        if (checkCar) {
            if (req.body.is_services == false) {
                ...
                var checkBooking = await Booking.findOne({ car: checkCar._id, date: new Date(req.body.date).toISOString(), status: { $in: ["Confirmed", "Pending", "In-Process", "Approval", "Approved", "Failed", "JobInitiated", "JobOpen", "EstimatePrepared", "ApprovalAwaited", "IntimationSent", "SurveyorAssigned", "SurveyorApproved", "StartWork", "CloseWork", "CompleteWork", "Rework", "Ready"] }, is_services: true }).exec();
...
router.post('/approved/services/add', xAccessToken.token, async function (req, res, next) {
    ...
    if (validation.fails()) {
        ...
        var booking = await Booking.findOne({ _id: req.body.booking, user: loggedInDetails._id }).exec();
        if (booking) {
            var services = req.body.services;
            ...
            for (var i = 0; i < services.length; i++) {
                ...
                if (parts.length > 0) {
                    ...
                    if (services[i].part_cost != 0) {
                        parts_visible = false;
                        var tax_info = await Tax.findOne({ tax: "28.0% GST" }).exec();
                        var tax_rate = tax_info.detail;
                ...
                bookingService.push({
                    ...
                    of_cost: Math.ceil(services[i].of_cost),
            ...
            if (approved.length > 0) {
                ...
                var labour_cost = _.sumBy(approved, x => x.labour_cost * x.quantity);
                var part_cost = _.sumBy(approved, x => x.part_cost * x.quantity);
                var of_cost = _.sumBy(approved, x => x.of_cost * x.quantity);
                ...
                var due = labour_cost + part_cost + of_cost + discount_total + policy_clause + salvage + pick_up_charges - paid_total;
                ...
                var payment = {
                    ...
                    policy_clause: parseFloat(policy_clause.toFixed(2)),
                    salvage: parseFloat(salvage.toFixed(2)),
                ...
                var date = new Date();
                if (booking.advance == true) {
                    var stage = "Estimation"
                    var status = "ApprovalAwaited";
                }
                else {
                    var stage = "In-Process";
                    var status = "StartWork";
                }

                Booking.findOneAndUpdate({ _id: booking._id }, { $set: { status: status, services: bookingService, payment: payment, due: due, updated_at: new Date() } }, { new: false }, async function (err, doc) {
                    if (err) {
                        res.status(400).json({
                            responseCode: 400,
...
router.post('/booking/package/add/', xAccessToken.token, async function (req, res, next) {
    ...
    if (validation.fails()) {
        ...
        if (car && checkPackage) {
            var checkBooking = await Booking.findOne({ car: car._id, date: new Date(req.body.date).toISOString(), status: { $in: ["Confirmed", "Pending", "Rejected", "In-Process", "Approval", "Approved", "Failed", "JobInitiated", "JobOpen", "EstimatePrepared", "ApprovalAwaited", "IntimationSent", "SurveyorAssigned", "SurveyorApproved", "StartWork", "CloseWork", "CompleteWork", "Rework", "Ready"] }, is_services: true }).exec();
...
router.get('/service/gallery/get', xAccessToken.token, async function (req, res, next) {
    var rules = {
        id: 'required',
    };
    ...
    if (validation.fails()) {
        ...
        if (booking) {
            ...
            if (booking.payment.total != 0 && booking.package == null) {
                ...
                var total = labour_cost + part_cost + of_cost + discount_total + policy_clause + salvage + pick_up_charges;
                var paid_total = part_cost + labour_cost + of_cost + policy_clause + salvage + pick_up_charges;
                ...
                Booking.findOneAndUpdate({ _id: booking._id }, { $set: { services: bookingService, payment: payment, due: null, updated_at: new Date() } }, { new: false }, async function (err, doc) {
                    if (err) {
                        res.status(400).json({
                            responseCode: 400,
                            responseMessage: "Error",
 ...
 * @param  {[raw json]}   req  [description]
 * @param  {[json]}   res  [description]
 transaction/* @param  {middleware} next [description]
 * @return {[json]}        [description]
*/
                                    ...
                                    booking.services.forEach(async function (service) {
                                        if (service.type == "package") {
                                            ...
                                            if (check <= 0) {
                                                ...
                                                var cashback = Offer.findOne({ source: package._id }).exec();
                                                if (cashback) {
                                                    if (cashback)
                                                        var point = {
                                                            user: booking.user,
...
router.get('/orders/get', xAccessToken.token, async function (req, res, next) {
    ...
    await Booking.find({ 'user': user, status: { $in: ["Cancelled", "Confirmed", "Pending", "Rejected", "Closed", "Completed", "Failure", "In-Process", "Dissatisfied", "Approval", "Approved", "Failed", "JobInitiated", "JobOpen", "EstimatePrepared", "ApprovalAwaited", "StartWork", "CloseWork", "CompleteWork", "Rework"] }, is_services: true })
...
function generateChecksum(param) {
