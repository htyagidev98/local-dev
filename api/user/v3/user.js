    ...
    if (validation.fails()) {
        ...
        if (checkCar && checkPackage) {
            ...
            await Service.find(cond).cursor().eachAsync(async (service) => {
                if (service) {
                    var labour_cost = await q.all(packageDiscount(req.body.package, checkCar._id, "services", service.service, service.labour_cost));
            ...
            await Collision.find({ service: req.body.label, model: checkCar.model }).cursor().eachAsync(async (service) => {
                if (service) {
                    var labour_cost = await q.all(packageDiscount(req.body.package, checkCar._id, "collision", service.service, service.labour_cost));
            ...
            await Washing.find({ service: req.body.label, model: checkCar.model }).cursor().eachAsync(async (service) => {
                if (service) {
                    var labour_cost = await q.all(packageDiscount(req.body.package, checkCar._id, "washing", service.service, service.labour_cost));
...
router.post('/payment/gateway/request', xAccessToken.token, async function (req, res, next) {

    /*var text =  Object.keys(req.query).map(function(key) {
                    return encodeURIComponent(key) + '=' +
                    encodeURIComponent(req.query[key]);
                }).join('&');*/

    var m = crypto.createHash('md5');
    m.update("9A6D0625F016D91E1A94467B2BB1C732");
    var key = m.digest('');
    var iv = '\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f';
    var cipher = crypto.createCipheriv('aes-128-cbc', key, iv);
    /*var encoded = cipher.update(text.toString(), 'utf8', 'hex');
    encoded += cipher.final('hex');

    var formbody = '<form id="nonseamless" method="post" name="redirect" action="https://test.ccavenue.com/transaction/transaction.do?command=initiateTransaction"/> <input type="hidden" id="encRequest" name="encRequest" value="' + encoded + '"><input type="hidden" name="access_code" id="access_code" value="AVYT82GA63AD63TYDA"><script language="javascript">document.redirect.submit();</script></form>';*/
...
router.post('/payu/hash', xAccessToken.token, async function (req, res, next) {
    var data = "key|5937f928cbb2a51c4fe8c4c1|10|Shopping|Test|test@test.com|||||||||||salt";

    const secret = 'BillioDollarDreams';
...
router.post('/payu/success', async function (req, res, next) {
    ...
    await Booking.findOneAndUpdate({ booking_no: parseInt(req.body.productinfo) }, { $set: data }, { new: false }, async function (err, doc) {
        res.status(200).json({
            responseCode: 200,
            responseMessage: "Payment Success",
            responseData: {}
...
router.post('/payment/gateway/success', async function (req, res, next) {

    var booking = await Booking.findOne({ booking_no: req.body.id }).exec();

    if (booking) {
        if (req.body.order_status == "Success") {
            if (booking.payment.paid_total == parseFloat(req.body.amount)) {
                var d1 = booking.date;
                var date = new Date();
                ...
                await Booking.findOneAndUpdate({ booking_no: req.body.id }, { $set: data }, { new: false }, async function (err, doc) {
                    if (err) {
                        res.status(422).json({
                            responseCode: 422,
                            responseMessage: "Error Occurred",
                        ...
                        if (booking.payment.discount_type == "coins") {
                            ...
                            Point.create(point).then(async function (point) {
                                await User.findOneAndUpdate({ _id: booking.user }, { $set: { careager_cash: remain } }, { new: false }, async function (err, doc) { });
                            })
                        }
                        ...
                        if (booking.is_services == true) {
                            ...
                            fun.newNotification(notify);
                            event.bookingMail(booking.user, booking.business, booking._id, booking.is_services);
                            ...
                            if (booking.advisor) {
                                ...
                                fun.newNotification(notify);
                                event.bookingMail(booking.user, advisor._id, booking._id, booking.is_services);
                            }
                ...
                res.status(422).json({
                    ...
                    responseData: {
                        paid: booking.payment.paid_total,
            ...
            await Booking.findOneAndUpdate({ _id: req.body.id }, { $set: data }, { new: false }, async function (err, doc) {
                if (err) {
                    ...
                    if (booking.is_services == true) {
                        ...
                        fun.newNotification(notify);
                        event.bookingMail(user, booking.business, booking._id, booking.is_services);
                        ...
                        if (booking.advisor) {
                            ...
                            fun.newNotification(notify);
                            event.bookingMail(user, advisor._id, booking._id, booking.is_services);
                        }
...
router.post('/roadside/claim-imitation', xAccessToken.token, async function (req, res, next) {
    ...
    if (count == 1) {
        ...
        data.business = "5beaa755db4f6610c635df4c";
...
router.get('/purchased/packages/get', xAccessToken.token, async function (req, res, next) {
    ...
    await UserPackage.find({ user: user })
        ...
        .cursor().eachAsync(async (package) => {
            ...
            if (package.category == "free") {
                var d = package.updated_at;
                var e = d.setDate(d.getDate() + 30);

            } else {
                var p = package.created_at;
                var e = package.expired_at;
            }
