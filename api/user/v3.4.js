...
//Abhinav Changes to Lead From App
            ...
            User.create(req.body).then(async function (user) {
                ...
                if (referral_code) {
                    ...
                    if (checkReferralCode) {
                        ...
                        if (user.agent.agent) {
                            whatsAppEvent.referralWhatsAppEventAgent(user.name, checkReferralCode.contact_no)
                        } else {
                            whatsAppEvent.referralWhatsAppEventUser(user.name, checkReferralCode.contact_no)
                        }
                ...
                await whatsAppEvent.androidUserReg(user.otp, user.contact_no)
                res.status(200).json({
                    responseCode: 200,
...
router.post('/address/add', async function (req, res, next) {
    // console.l.log("Address add....", req.body)
    var rules = {
        address: 'required',
        zip: 'required',
        city: 'required',
...
router.get('/address/get', async function (req, res, next) {
    var token = req.headers['x-access-token'];
    var secret = config.secret;
    var decoded = jwt.verify(token, secret);
    var user = decoded.user;
            ...
            process.then(function (video) {
                video.fnExtractFrameToJPG('/tmp/', {
                    ...
                    if (!error) {
                        ...
                        //// console.l.log({ status: true , message: "Video thumbnail created."});
                    }
                    else {
                        //// console.l.log(error)
                    }
...
router.get('/booking/category/get', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Booking category get", req.query)
    var data = [];
    var token = req.headers['x-access-token'];
    var secret = config.secret;
    var decoded = jwt.verify(token, secret);
...
router.post('/booking/services/get', async function (req, res, next) {
    // console.l.log("Booking service get...", req.body)
    var rules = {
        car: 'required',
    };
...
router.get('/get/booking/cars/', async function (req, res, next) {
    // console.l.log("user contact number....", req.query.contact)
    let contact = req.query.contact
    let user = await User.findOne({ contact_no: contact }).exec()

    cars = []
    if (user) {

        for (let i = 0; i < user.cars.length; i++) {
            let getCar = await Car.findOne({ _id: mongoose.Types.ObjectId(user.cars[i]) }).exec()
            cars.push(getCar)
        }

    }
    // console.l.log("User details....", cars)
    return res.json({
        responseCode: "200",
        cars: cars
    })
...
router.post('/web/booking/add/', async function (req, res, next) {
    // console.l.log("Logs body..............................", req.body.status)
    var rules = {
        car: 'required',
        services: 'required'
    };
    let newCarId = undefined;
    var validation = new Validator(req.body, rules);
    ...
    if (validation.fails()) {
        ...
        if (!checkCar) {
            ...
            var variant = await Variant.findOne({ _id: req.body.car }).select('-service_schedule').exec();
            var currentDate = new Date();
            if (variant != null && variant) {
                var rg = req.body.registration_no;
                var registration_no = Math.round(+new Date() / 1000) + Math.round((Math.random() * 9999) + 1);
                var newCar = new Car({
                ...
                //// console.l.log(req.body.transmission)
                // vinay new car

                await Car.create(newCar).then(async function (car) {
                    newCarId = car._id
        ...
        if (baz > 0) {
            ...
            if (checkVendor) {
                ...
                if (doorstep) {
                    ...
                    // status: "Assigned"
                    var lastBooking = await Booking.findOne({ user: user, $or: [{ status: { $in: ["Inactive", "Assigned"] } }] }).sort({ created_at: -1 }).exec();
                    var status = 'Inactive';
                    if (req.body.status == "pay_later") {
                        status = "Confirmed"
                    ...
                    if (lastBooking) {
                        ...
                        // console.l.log("Line 4642")
                        var data = {
                            // need to change this car
                            package: req.body.package,
                            car: newCarId,
                            advisor: advisor,
                        ...
                        await Booking.findOneAndUpdate({ _id: lastBooking._id }, { $set: data }, { new: true }, async function (err, doc) {
                            if (!err) {
                                // console.l.log("Line 4667")

                                var booking = await Booking.findById(lastBooking._id).exec();
                                if (booking.is_services == true) {
                                    // console.l.log("Previous booking called...", lastBooking._id)
                                    if (req.body.status == "pay_later") {
                                        await whatsAppEvent.bookingWhatsApp(req.body.business, lastBooking._id)
                                    }
                        ...
                        await Booking.create(data).then(async function (booking) {
                            if (booking.is_services == true) {
                                // console.l.log("New booking create..", booking._id)
                                if (req.body.status == "pay_later") {
...
router.post('/booking/add/', async function (req, res, nexst) {
    // console.log("Booking add....", req.body)
    var rules = {
        car: 'required',
        services: 'required'
    };
...
//Sumit
router.post('/booking/add/error', async function (req, res, nexst) {
    // console.l.log("Booking add....", req.body)
    var rules = {
        car: 'required',
        services: 'required'
    };
...
router.post('/booking/package/add/', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Booking package add......", req.body)
    var rules = {
        package: 'required',
    };
        ...
        if (car && checkPackage) {
            ...
            if (baz > 0) {
                ...
                await Management.find({ business: checkPackage.business, role: "Service Advisor" })
                    .cursor().eachAsync(async (a) => {
                        var d = await Booking.find({ business: checkPackage.business, advisor: a.user, status: { $in: ["EstimateRequested", "JobInitiated", "JobOpen", "In-Process", "StartWork", "Rework"] } }).count().exec();
                        advisorBooking.push({
                            user: a.user,
                            count: await Booking.find({ business: checkPackage.business, advisor: a.user, status: { $in: ["EstimateRequested", "JobInitiated", "JobOpen", "In-Process", "StartWork", "Rework"] } }).count().exec()
                        })
                    });
...
router.post('/approved/services/add', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Approved service add....", req.body)
    var rules = {
        booking: 'required',
    };
        ...
        if (booking) {
            ...
            for (var i = 0; i < services.length; i++) {
                ...
                if (parts.length > 0) {
                    ...
                    // console.l.log(services[i].parts)
                }
                else {
...
router.get('/service/description/get', xAccessToken.token, async function (req, res, next) {
    ...
    // console.l.log("Service description get....", req.query)
    var rules = {
        id: 'required',
    };
...
router.get('/service/gallery/get', async function (req, res, next) {
    ...
    // console.l.log("Service gallery get...", req.query)
    var rules = {
        id: 'required',
    };
...
router.post('/bookings/time-slot/', async function (req, res, next) {
    // console.l.log("Bookings time slot.....", req.body)
    var rules = {
        business: 'required',
        date: 'required',
    };
...
router.post('/booking/coupon/add', async function (req, res, next) {
    // console.l.log("Booking coupon add...", req.body)
    var rules = {
        id: 'required',
    };
        ...
        if (booking) {
            ...
            if (booking.payment.total != 0 && booking.package == null) {
                ...
                if (type == "coupon") {
                    ...
                    var coupon = await Coupon.findOne({ code: req.body.coupon.toUpperCase(), is_product: false, publish: true }).exec();
...
router.post('/booking/coupon/remove', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Booking coupon remove....", req.body)
    var rules = {
        id: 'required',
        type: 'required',
    };
...
router.get('/booking/coupon/list', async function (req, res, next) {
    ...
    var data = [];
    var business = null;
    // console.l.log("user = " + user)

    if (req.query.business) {
        business = req.query.business
    }
    await Coupon.findOne({ is_product: false, business: business, publish: true })
...
router.get('/booking/convenience', async function (req, res, next) {
    var token = req.headers['x-access-token'];
    var secret = config.secret;
    var decoded = jwt.verify(token, secret);
    var data = [];
    ...
    var conveniences = [];
    // business: req.query.business
    var check = await BusinessConvenience.find({}).count().exec();
...
router.get('/payment/checksum/generate', async function (req, res, next) {
    var token = req.headers['x-access-token'];
    var secret = config.secret;
    var decoded = jwt.verify(token, secret);
    var user = decoded.user;
...
router.post('/payment/success', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Payment success...", req.body)
    var rules = {
        id: 'required',
    };
...
router.post('/pay/later', xAccessToken.token, async function (req, res, next) {
    // console.l.log("pay later...", req.body)
    var rules = {
        id: 'required',
    };
...
router.post('/cart/add', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Cart Add....", req.body)
    var rules = {
        product: 'required',
        business: 'required',
    };
...
router.put('/cart/service/add', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Cart service add...", req.body)
    var rules = {
        item: 'required',
        services: 'required',
    };
...
router.post('/order/time-slot/', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Order time slot...", req.body)
    var slots = [];
    var date = new Date(new Date(req.body.date).setHours(0, 0, 0, 0));
    var next = new Date(new Date(req.body.date).setHours(0, 0, 0, 0));
    next.setDate(date.getDate() + 1);
...
router.get('/order/conveniences/get', xAccessToken.token, async function (req, res, next) {
    // console.l.log("Orders....", req.query);
    var business = [];
...
router.get('/orders/get', xAccessToken.token, async function (req, res, next) {
    ...
    // console.l.log("Orders get....", req.query)
    var token = req.headers['x-access-token'];
    var secret = config.secret;
    var decoded = jwt.verify(token, secret);
...
router.post('/bookings/time-slot/xpress', async function (req, res, next) {
    var rules = {
        business: 'required',
        date: 'required',
    };
    // console.l.log("Called   ==  " + req.body.booking)
    var validation = new Validator(req.body, rules);
...
//For Testing Without Token
router.post('/pay/later/xpress', async function (req, res, next) {
    var rules = {
        id: 'required',
        user: 'required',
    };
    ...
    if (validation.fails()) {
        ...
        // var decoded = jwt.verify(token, secret);
        // var user = decoded.user;
        var loggedInDetails = await User.findById(req.body.user).exec();
        var booking = await Booking.findById(req.body.id).exec();
        var coupon = await Coupon.findOne({ code: req.body.coupon }).exec();
