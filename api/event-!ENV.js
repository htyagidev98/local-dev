...
module.exports = {
    registrationSms: function (user) {
        ...
        var hash = encodeURIComponent("b67a24364572312b830eddfa6a53294acaf3b4dd8c46f742a787d22b41d2e275");
    ...
    bookingMail: async function (b, business) {
        ...
        var careager = 'care@careager.com';
        var autroid = 'care@autroid.com';
        var careagerPass = 'XqD2h>DV';
    ...
    leadgen: async function (TotalLeads, names, email, d, tz) {
        ...
        let mail = {
            from: 'Autroid WMS <care@careager.com>',
    ...
    ServiceApproval: async function (ser, business, seg, pack, service, part_cost, labour_cost, type, tz) {
        ...
        if (business == "5bfec47ef651033d1c99fbca") {
            ...
            let mail = {
                ...
                // from: "My Bsuness" + '<care@careager.com>',
                //to: "abhinav@autroid.com",
    ...
    autroidSignUpMail: async function (user) {
        ...
        // console.log("Email = " + user.email)
        let mail = {
            from: 'Admin(AUTROID) <care@autroid.com>',
            to: user.email,
    ...
    uploadToS3Estimate: async (body, filename, b, activity, preEstiamtePDF) => {
        var s3 = new aws.S3({
            accessKeyId: config.IAM_USER_KEY,
            secretAccessKey: config.IAM_USER_SECRET,
            Bucket: config.BUCKET_NAME
        ...
        if (preEstiamtePDF != '') {
            ...
            s3.deleteObject(estimatParams, function (err, data) {
                ...
                else console.log("Deleted =" + JSON.stringify(data, null, '\t'));                 // deleted
            });
        }
        filename = uuidv1()
        var params = {
        ...
        await s3.upload(params, async function (err, response) {
            ...
            await Booking.findOneAndUpdate({ _id: mongoose.Types.ObjectId(b) }, { $set: data }, { new: true }, async function (err, doc) {
                ...
                if (err) {
                    ...
                    console.log("sss==" + JSON.stringify(doc.estimate_pdf, null, '\t'));
                    console.log("END Time = " + new Date().toISOString())
                    // console.log("Start Time = " + new Date().toISOString())
                    if (activity == 'email') {
                        // sendEstimateMail(b, doc.business);
                        // return 'Abhi'
                        let title = doc.car.title;
                        let due = doc.due.due;

                        let email = `${process.env.careager_email}`
                        let pass = `${process.env.careager_pass}`
                        let transporter = nodemailer.createTransport({
                            host: 'smtp.gmail.com',
                            port: 465,
                            secure: true,
                    ...
                    if (activity == 'whatsapp') {
                        console.log("doneeee");
                        whatsAppEvent.estiamteSendWhatsapp(b, doc.business);
                    }
                    // console.log(doc);
                }
            })
